...
async function main() { 
    ...
    async function setAsCurr(pokemon) {
        ...
        curr_atk.innerHTML = `atk - ${await getStats(pokemon).atk}`
        curr_def.innerHTML = `def - ${await getStats(pokemon).def}`
        curr_satk.innerHTML = `satk - ${await getStats(pokemon).satk}`
        curr_sdef.innerHTML = `sdef - ${await getStats(pokemon).sdef}`
        curr_speed.innerHTML = `speed - ${await getStats(pokemon).speed}`
    }
}main()